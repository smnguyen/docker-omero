...
while [ ! -d $(dirname $OMERO_DATA_DIR) ]; do
...
if [ -f "/data/config.sh" ]; then
    gosu omero bash /data/config.sh || (echo "Something failed while running config.sh"; exit 1);
fi


echo "Linking to data container volume"
